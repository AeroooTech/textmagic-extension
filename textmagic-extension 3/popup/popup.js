...
let activeCat = 'Alle';
let editingId  = null;
let pageVars   = []; // { name, selector, preview }
...
function openPanel(sn = null) {
  ...
  // Extract existing page vars from content
  if (sn) {
    const re = /\{\{page:([^}]+)\}\}/g;
    let m;
...
// The popup stays OPEN. We send a message to the background worker,
// which injects the content script and relays the picker to the active tab.
// When the user clicks an element, the result comes back here.
...
document.getElementById('btn-pick').onclick = async () => {
  ...
  // Check if we can inject (not chrome:// pages etc.)
  if (tab.url?.startsWith('chrome://') || tab.url?.startsWith('chrome-extension://')) {
    return alert('Picker funktioniert nicht auf internen Chrome-Seiten.\nBitte navigiere zu einer normalen Webseite.');
  }
  ...
  // Send to background relay (background will inject + forward to content script)
  chrome.runtime.sendMessage(
    ...
    (result) => {
      ...
      if (chrome.runtime.lastError || !result) {
        console.error('Picker error:', chrome.runtime.lastError);
        return;
      }

      // Add page var to list
...
function showPickerStatus() {
  document.getElementById('picker-status').classList.add('show');
}
function closePanelStatus() {
  document.getElementById('picker-status').classList.remove('show');
}
...
async function saveSettings() {
  ...
  // prefix already set before call
